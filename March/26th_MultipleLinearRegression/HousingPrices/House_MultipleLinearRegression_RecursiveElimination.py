...
# Ensure X_opt and Y have correct shapes
X_opt = np.array(X_opt, dtype=float)  # Convert to float if needed
Y = np.array(Y, dtype=float).reshape(-1, 1)  # Reshape Y if it's 1D
